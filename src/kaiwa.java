...
public class kaiwa implements State {
	...
	int time = 0;
	int timee = 0;
	int nameflag = 1;//1はアリス,2は帽子屋,3は空欄
	int tekitime = 0;
	int sound = 0;//フラグとして使用
	...
	public void processTimeElapsed(int msec) {
		if (model.i == 9) //「Push Enter」の部分にタイマー使用、ハチが来るときEnter押せなくなるのに使用
			tekitime += 10;
		timee++;
		if (timee >= 5) {
			time++;
	...
	public void paint(Graphics g) {
		...
		if (model.i == 4 || model.i == 6 || model.i == 8 || model.i == 11 || model.i == 12 || model.i == 14
				|| model.i == 15 || model.i == 16 || model.i == 17 || model.i == 19 || model.i == 23 || model.i == 25) {
			g.drawImage(bousi, 680, 240, model.getView());
			g.drawImage(arisukuro, 0, 300, model.getView());
			nameflag = 2;
		}

		if (model.i == 5 || model.i == 7 || model.i == 10 || model.i == 13 || model.i == 18 || model.i == 20
				|| model.i == 22 || model.i == 24 || model.i == 26) {
		...
		if (model.i == 14)
			g.drawString(kaiwaa[model.i], 160, 703);
		else
			g.drawString(kaiwaa[model.i], 170, 703);
